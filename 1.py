...
class PaintApp:
    def __init__(self):
        ...
        pygame.display.set_caption("Paint Application - Fullscreen")
        ...
        #чёрный, красный, зелёный, синий, жёлтый, пурпурный, голубой, белый
    ...
    def update_brush_sizes(self):
        for tool_name, tool in self.tools.items():
            ...
            if hasattr(tool, 'color') and tool_name != "eraser":
    ...
    def handle_events(self):
        for event in pygame.event.get():
            ...
            if self.current_tool == "text" and self.tools["text"].active:
                if event.type == pygame.KEYDOWN:
                    result = self.tools["text"].handle_text_input(event, self.canvas)
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                ...
                if self.is_drawing and on_canvas:
                    result = self.tools[self.current_tool].handle_event(self.canvas, canvas_pos, mouse_pressed)
                    if result:
                        self.canvas = result
                    self.is_drawing = False
        ...
        if canvas_pos[0] < self.canvas_width:
            if self.current_tool not in ["flood_fill"]:
                if hasattr(self.tools[self.current_tool], 'draw_preview'):
                    self.tools[self.current_tool].draw_preview(preview)
        
        if self.current_tool == "text" and self.tools["text"].active:
            self.tools["text"].draw_preview(preview)
